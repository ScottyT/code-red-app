        ...
        <v-card light color="white" max-width="344" elevation="2" class="user-card">
            ...
            <form enctype="multipart/form-data" @submit.prevent="uploadFile">
                ...
                <template v-slot:activator>
                    <v-avatar size="150px" v-ripple v-if="Object.keys(avatar).length === 0 || !avatar" class="grey lighten-3 mb-3">
                        Click to add avatar
                    </v-avatar>
                    <v-avatar size="150px" v-ripple v-else-if="avatar.imageUrl" class="mb-3">
...
export default {
    setup(props, {root}) {
        ...
        const { avatar, fetchAvatar } = useUsers();
    ...
    methods: {
        uploadFile() {
            this.avatar.formData.append('teamMember', this.userObj.email)
